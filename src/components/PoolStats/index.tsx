...
const UserStatsGrid = styled.div<{ isDarkTheme: boolean }>`
  ...
  border-radius: 16px;
  background: ${(props) =>
    props.isDarkTheme ? "rgba(255, 255, 255, 0.03)" : "rgba(41, 88, 255, 0.05)"};
  border: 1px solid
    ${(props) =>
      props.isDarkTheme ? "rgba(255, 255, 255, 0.08)" : "rgba(41, 88, 255, 0.15)"};
`;
...
const ExpandedRow = styled.tr<{ isDarkTheme: boolean }>`
  background-color: ${(props) =>
    props.isDarkTheme ? "#111827" : "#F9FAFB"};
  & > td {
    padding: 1rem 1.5rem;
...
const DebugSection = styled.div<{ isDarkTheme: boolean }>`
  ...
  padding: 0.75rem;
  background-color: ${(props) =>
    props.isDarkTheme ? "#1F2937" : "#FFFFFF"};
  border-radius: 0.375rem;
  border: 1px solid
    ${(props) => (props.isDarkTheme ? "#374151" : "#E5E7EB")};
`;
...
const DebugPre = styled.pre<{ isDarkTheme: boolean }>`
  ...
  color: ${(props) => (props.isDarkTheme ? "#9CA3AF" : "#4B5563")};
  background-color: ${(props) =>
    props.isDarkTheme ? "#111827" : "#F3F4F6"};
  padding: 0.5rem;
  border-radius: 0.25rem;
  overflow-x: auto;
`;

const SparklineContainer = styled.div<{ isDarkTheme: boolean; apr: number; pulseSpeed: number }>`
  width: 100px;
  height: 40px;
  display: flex;
  align-items: center;
  ...
  * {
  ...
  &::before {
    ...
    background: ${(props) => {
      const apr = props.apr;
      if (apr > 20) return props.isDarkTheme 
        ? 'rgba(16, 185, 129, 0.3)' 
        : 'rgba(16, 185, 129, 0.2)';
      if (apr > 10) return props.isDarkTheme 
        ? 'rgba(59, 130, 246, 0.25)' 
        : 'rgba(59, 130, 246, 0.15)';
      if (apr > 5) return props.isDarkTheme 
        ? 'rgba(251, 191, 36, 0.2)' 
        : 'rgba(251, 191, 36, 0.1)';
      return 'transparent';
    }};
    opacity: ${(props) => (props.apr > 5 ? 0.6 : 0)};
    animation: ${(props) => props.pulseSpeed > 0 ? `pulse ${props.pulseSpeed}s ease-in-out infinite` : 'none'};
    z-index: -1;
  }
...
const SparklineChart: React.FC<SparklineChartProps> = React.memo(({
    ...
    const fetchVolumeData = async () => {
      try {
        ...
        const poolTrades = [...(data.buy || []), ...(data.sell || [])].filter(
          (trade: any) => {
            const tradePoolId = trade.contract_id || trade.pool_id || trade.poolId;
            return (
              `${tradePoolId}` === `${poolId}` ||
              `${tradePoolId}` === poolId ||
        ...
        console.log(`[SparklineChart] Pool ${poolId} - Found ${poolTrades.length} trades`);
        ...
        const groupedData = poolTrades.reduce(
          (acc: Record<string, any>, trade: any) => {
            ...
            const baseVol = parseFloat(trade.base_volume || trade.baseVolume || "0");
            const targetVol = parseFloat(trade.target_volume || trade.targetVolume || "0");
            // Use USD volume if available, otherwise approximate
            const usdVolume = parseFloat(trade.usd_volume || trade.usdVolume || "0") || 
        ...
        console.log(`[SparklineChart] Pool ${poolId} - Generated ${chartDataArray.length} data points`);
        ...
        if (!cancelled) {
          console.error(`[SparklineChart] Pool ${poolId} - Error fetching chart data:`, error);
          setChartData([]);
          setLoading(false);
        }
      }
  ...
  if (loading) {
    return (
      <SparklineContainer isDarkTheme={isDarkTheme} apr={aprValue} pulseSpeed={0}>
  ...
  if (validChartData.length < 2) {
    ...
    if (volume24h?.usdVolume) {
      ...
      if (placeholderData.length >= 2) {
        ...
        const lastVal = placeholderData[placeholderData.length - 1]?.value || 0;
    ...
    if (aprValue > 0) {
      const aprColor = aprValue > 20 ? "#10B981" : aprValue > 10 ? "#3B82F6" : aprValue > 5 ? "#FBBF24" : "#6B7280";
      return (
        <SparklineContainer isDarkTheme={isDarkTheme} apr={aprValue} pulseSpeed={0}>
          <div style={{ 
            display: "flex", 
        ...
        <div style={{
          ...
          <svg width="80" height="30" viewBox="0 0 80 30" style={{ overflow: "visible" }}>
            <line
              ...
              strokeDasharray="2 2"
            />
            <circle cx="5" cy="25" r="2" fill={isDarkTheme ? "#374151" : "#D1D5DB"} />
            <circle cx="75" cy="25" r="2" fill={isDarkTheme ? "#374151" : "#D1D5DB"} />
          </svg>
        </div>
      </SparklineContainer>
    );
  ...
  return (
    <SparklineContainer isDarkTheme={isDarkTheme} apr={aprValue} pulseSpeed={pulseSpeed}>
      <ResponsiveContainer width="100%" height="100%">
...
const PoolStats: React.FC = () => {
  ...
  const calculateTotalAprForStat = useCallback((stat: PoolStatData): number => {
  ...
  const getAprBreakdown = useCallback((stat: PoolStatData) => {
    const baseApr = parseFloat(stat.fees.apr || "0");
    const poolIdNum = Number(stat.poolId);
  ...
  const validStats = useMemo(() => {
    ...
    const filtered = statsData.stats.filter(
      (s) => parseFloat(s.tvl.usd) > 0
    );
    ...
    const sorted = [...filtered].sort((a, b) => {
      switch (sortBy) {
        ...
        case "volume":
          return parseFloat(b.volume["24h"].usdVolume || "0") - parseFloat(a.volume["24h"].usdVolume || "0");
        case "fees":
          return parseFloat(b.fees["24hFeesUSD"] || "0") - parseFloat(a.fees["24hFeesUSD"] || "0");
        case "apr":
        ...
        <LayoutGrid>
          <MainColumn>
            <Panel isDarkTheme={isDarkTheme}>
              <PanelHeader>
                <div>
                  <PanelTitle isDarkTheme={isDarkTheme}>Your positions</PanelTitle>
                  <PanelSubtitle isDarkTheme={isDarkTheme}>
                    Provide liquidity to start earning fees and on-chain rewards.
                  </PanelSubtitle>
                </div>
                <HeroCTA isDarkTheme={isDarkTheme} onClick={() => navigate("/pool/create")}>
                  New position
                </HeroCTA>
              </PanelHeader>
              ...
              <UserStatsGrid isDarkTheme={isDarkTheme}>
                <StatItem>
                  ...
                  <StatValue isDarkTheme={isDarkTheme}>{filteredPositions.length}</StatValue>
                </StatItem>
              </UserStatsGrid>
              ...
              {filteredPositions.length > 0 ? (
                    ...
                    <div style={{ display: "flex", justifyContent: "center", gap: "8px", marginTop: "16px" }}>
                      <button
                        ...
                        style={{
                          padding: "8px 16px",
                          borderRadius: "8px",
                          border: `1px solid ${isDarkTheme ? "rgba(255, 255, 255, 0.15)" : "rgba(41, 88, 255, 0.15)"}`,
                        ...
                        if (pageItem === "ellipsis") {
                          return (
                            <span
                              ...
                              style={{
                                padding: "0 4px",
                                color: isDarkTheme ? "rgba(255, 255, 255, 0.5)" : "rgba(12, 12, 16, 0.5)",
                                fontSize: "14px",
                              }}
                            >
                              ...
                        ...
                        return (
                          <button
                            ...
                            style={{
                              padding: "8px 12px",
                              borderRadius: "8px",
                              border: `1px solid ${page2 === pageItem ? "#2958ff" : isDarkTheme ? "rgba(255, 255, 255, 0.15)" : "rgba(41, 88, 255, 0.15)"}`,
                              background: page2 === pageItem ? "#2958ff" : isDarkTheme ? "#050507" : "#ffffff",
                              color: page2 === pageItem ? "#fff" : isDarkTheme ? "#fff" : "#0c0c10",
                              cursor: "pointer",
                            }}
                          >
                            {pageItem}
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background: isDarkTheme ? "#050507" : "#ffffff",
                          color: isDarkTheme ? "#fff" : "#0c0c10",
                          cursor: page2 === totalPages ? "not-allowed" : "pointer",
                ...
                <EmptyState isDarkTheme={isDarkTheme}>
                  You don't have liquidity positions yet. Create one to see it here.
                </EmptyState>
              )}
            </Panel>
          </MainColumn>
          ...
          <SideColumn>
            <Panel isDarkTheme={isDarkTheme}>
              <PanelHeader>
                <div>
                  <PanelTitle isDarkTheme={isDarkTheme}>Pools with rewards</PanelTitle>
                  <PanelSubtitle isDarkTheme={isDarkTheme}>
            ...
            <Panel isDarkTheme={isDarkTheme}>
              <PanelHeader>
                <div>
                  <PanelTitle isDarkTheme={isDarkTheme}>Top pools by TVL</PanelTitle>
                  <PanelSubtitle isDarkTheme={isDarkTheme}>
            ...
            <Panel isDarkTheme={isDarkTheme}>
              <PanelHeader>
                <div>
                  <PanelTitle isDarkTheme={isDarkTheme}>Top pools by Volume</PanelTitle>
                  <PanelSubtitle isDarkTheme={isDarkTheme}>Highest 24h trading volume pools.</PanelSubtitle>
                </div>
              </PanelHeader>
              <SimpleList>
            ...
            {validStats.map((stat) => {
              ...
              return (
                <React.Fragment key={stat.poolId}>
                  ...
                  >
                    <TableCell isDarkTheme={isDarkTheme} data-label="Pair" className="expand-cell">
                      <PairCell>
                        {hasDebugInfo && (
                          <ExpandIcon
                            isExpanded={isExpanded}
                            className="expand-icon"
                            onClick={(e) => toggleRowExpansion(stat.poolId, e)}
                            style={{ cursor: "pointer" }}
                          >
                            ▶
                          </ExpandIcon>
                        ...
                        />
                        <span style={{ fontWeight: 500 }}>{tokenASymbol}</span>
                        <span>/</span>
                        <TokenIcon
                        ...
                        />
                        <span style={{ fontWeight: 500 }}>{tokenBSymbol}</span>
                      </PairCell>
                    </TableCell>
                    <TableCell isDarkTheme={isDarkTheme} data-label="Pool ID">
                      {stat.poolId}
                    </TableCell>
                    <TableCell isDarkTheme={isDarkTheme} data-label="TVL (USD)">
                    ...
                    <TableCell isDarkTheme={isDarkTheme} data-label="24h Fees">
                    ...
                    <TableCell isDarkTheme={isDarkTheme} data-label="APR">
                      {(() => {
                        ...
                        const tooltipContent = (
                          <div>
                            <p><strong>Total APR: {breakdown.totalApr.toFixed(2)}%</strong></p>
                            <p>Base Swap APR: {breakdown.baseApr.toFixed(2)}%</p>
                            {breakdown.aprBoost > 0 && (
                              <p>DeFi Boost: {breakdown.aprBoost.toFixed(2)}%</p>
                            )}
                            {breakdown.blockReward > 0 && (
                              <p>Block Rewards: {breakdown.blockReward.toFixed(2)}%</p>
                            )}
                            {breakdown.additionalAprBoost > 0 && (
                              <p>Additional APR Boost: {breakdown.additionalAprBoost.toFixed(2)}%</p>
                            )}
                            {!breakdown.rewardFound && breakdown.totalApr === breakdown.baseApr && (
                              ...
                              {stat.debug.calculations && (
                                <div>
                                  ...
                                  <DebugPre isDarkTheme={isDarkTheme}>
                                    {JSON.stringify(stat.debug.calculations, null, 2)}
                                  </DebugPre>
                                </div>
                              )}
                            </DebugSection>
                            ...
                            <DebugSection isDarkTheme={isDarkTheme}>
                              ...
                              <div style={{ marginBottom: "0.5rem" }}>
                                ...
                                <div style={{ marginLeft: "0.5rem", marginTop: "0.25rem" }}>
                                  Token A: {formatNumber(stat.poolInfo.poolBals.A)} ({stat.tokens.tokenA.unitName})
                                  <br />
                                  Token B: {formatNumber(stat.poolInfo.poolBals.B)} ({stat.tokens.tokenB.unitName})
                                </div>
                              </div>
                              <div style={{ marginBottom: "0.5rem" }}>
                                <strong>LP Tokens:</strong>
                                <div style={{ marginLeft: "0.5rem", marginTop: "0.25rem" }}>
                                  Minted: {formatNumber(stat.poolInfo.lptBals.lpMinted)}
                                  <br />
                                  Held: {formatNumber(stat.poolInfo.lptBals.lpHeld)}
                                </div>
                              </div>
                              <div>
