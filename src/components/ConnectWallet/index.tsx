...
import PoolIcon from "../../components/SVG/Pool";
import TokenIcon from "../../components/SVG/Token";
...
const AccountName = styled.span<{ $isDarkTheme?: boolean }>`
...
const ActiveButton = styled.div<{ $isDarkTheme?: boolean }>`
  display: flex;
  padding: 6px 12px;
  justify-content: center;
  align-items: center;
...
const ConnectButton = () => {
  return (
    <svg
      ...
      xmlns="http://www.w3.org/2000/svg"
    >
      <g filter="url(#filter0_d_636_14464)">
      ...
      <defs>
        ...
        >
          ...
          <feBlend
            ...
            result="effect1_dropShadow_636_14464"
          />
          <feBlend
            mode="normal"
            in="SourceGraphic"
            in2="effect1_dropShadow_636_14464"
            result="shape"
          />
        </filter>
      </defs>
...
const DrawerContent = styled.div<{ $isMobile: boolean; $isOpen: boolean }>`
  ...
  min-height: ${(props) => (props.$isMobile ? "200px" : "auto")};
  max-height: ${(props) => (props.$isMobile ? "calc(100% - 80px)" : "100%")};
  display: flex;
  flex-direction: column;
  position: relative;
  z-index: 1301;
...
const IconButton = styled.button<{
  ...
  color: ${(props) => (props.$isDarkTheme ? "#FFFFFF" : "#161717")};
  opacity: 0.7;
  width: ${(props) => props.$width || (props.$expanded ? "120px" : "40px")};
...
const DisconnectLabel = styled.span<{ $isDarkTheme?: boolean }>`
  color: ${(props) => (props.$isDarkTheme ? "#FFFFFF" : "#161717")};
  font-size: 14px;
  font-weight: 500;
  white-space: nowrap;
...
const PowerIcon = styled(PowerSettingsNewIcon)<{ $expanded?: boolean }>`
...
const SettingsMenu = styled.div<{ $isDarkTheme?: boolean }>`
  position: absolute;
  top: 100%;
  right: 0;
  width: 200px;
...
const SettingsMenuItem = styled.div<{ $isDarkTheme?: boolean }>`
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 12px 16px;
...
// Add new styled component for the nested menu
const SwapSettingsModal = styled(Modal)<{ $isDarkTheme?: boolean }>`
  .MuiBackdrop-root {
...
function BasicMenu() {
  ...
  // Add new state for swap settings menu
  const [isSwapModalOpen, setIsSwapModalOpen] = React.useState<boolean>(false);

  // Add ref for the power button
  const powerButtonRef = React.useRef<HTMLButtonElement>(null);

  // Add ref for settings button
  const settingsButtonRef = React.useRef<HTMLButtonElement>(null);

  // Update theme toggle handler to not close the menu
  const handleThemeToggle = () => {
    dispatch(toggleTheme());
  };

  // Update settings click handler to toggle only if clicking the button
  const handleSettingsClick = (event: React.MouseEvent) => {
    event.stopPropagation(); // Prevent event from bubbling to drawer backdrop
    setShowSettings(!showSettings);
  };

  // Update click outside effect to be more specific
  React.useEffect(() => {
  ...
  const handlePowerClick = (event: React.MouseEvent) => {
    event.stopPropagation(); // Prevent event from bubbling
    if (isPowerExpanded) {
      activeWallet?.disconnect();
      handleClose();
    } else {
      setIsPowerExpanded(true);
    }
  };

  // Add effect to collapse both buttons when drawer closes
  React.useEffect(() => {
    if (!isDrawerOpen) {
  ...
  // Add effect for handling clicks outside the power button
  React.useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (
        isPowerExpanded &&
  ...
  return (
    <div>
      {!activeAccount ? (
        ...
        <AccountDropdown onClick={() => setIsDrawerOpen(true)}>
          ...
          >
            <Box
              sx={{
                width: "100%",
                height: "100%",
                background: isDarkTheme ? "#20093E" : "#FFFFFF",
                ...
                <DrawerHeader $isDarkTheme={isDarkTheme}>
                  ...
                  <ButtonGroup>
                    ...
                    <IconButton
                      ref={powerButtonRef}
                      $isDarkTheme={isDarkTheme}
                      $expanded={isPowerExpanded}
                      onClick={handlePowerClick}
                    >
                      <PowerIcon $expanded={isPowerExpanded} />
                      {isPowerExpanded && (
                        <DisconnectLabel $isDarkTheme={isDarkTheme}>
                          Disconnect
                        </DisconnectLabel>
                      )}
                    </IconButton>
                  ...
                  >
                    ...
                    <ConnectedAccountContainer>
                      {wallet.accounts.map((account) => {
                        return (
                          <AccountContainer key={account.address}>
                            <AccountNameContainer>
                              <AccountName $isDarkTheme={isDarkTheme}>
